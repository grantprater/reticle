...
# Asking the player, instead of deriving it
...
attempt measured, plausible and wrong — and the player named the defect by eye in
...
the player knows what is drawn on the screen; the detector only knows what it found.
...
marks AND a dropped spike AND ally portraits. the player, reading a contact sheet
...
* `root.bind("bracketleft", ...)` binds a *sequence of eleven keypresses*. A
  ...
  (`"q"`) is fine bare. This silently broke brush sizing and the player painted a
...
* A context panel that resizes 1080p to a tile's height is unreadable — the player
