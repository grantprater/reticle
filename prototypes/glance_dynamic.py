...
* **VALID** -- the channel survives the map change at this magnification, and my
  answers are worth putting to the player as a pre-screen rather than a label set;
...
`labels/minimap_dynamic/` -- that file takes the answers only, and seeding it
...
# answer and a label row are the same string. `warning_ping` is NEW, from the player
...
def controls_from(fit: str, n: int, rng, per_class_max: int = 2):
    """the rows on the fit session, re-cropped from its video.

    Only `by == human` rows are eligible, and `uncertain` ones are skipped --
    a control whose truth the player himself hedged measures nothing. Spread across
    ...
    pool = [r for r in last.values()
            if r.get("by") == "human" and not r.get("uncertain")]
    by_class: dict = {}
    for r in pool:
...
def main(argv=None) -> int:
    ...
    sheet = glance.present(
        ...
        truth_source=f"human, labels/minimap_dynamic/{a.fit}.jsonl (by=human, not uncertain)",
