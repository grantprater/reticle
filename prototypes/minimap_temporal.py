...
own. The minimap has NO camera motion: the player runs it fixed rotation, one
...
* **Omen's ultimate** turns the minimap a fuzzy black for a few seconds, and
  the player reports it is the only ability in the game that does this -- a bounded
...
def motion(track, per_frame):
    ...
    the point, and it is the discriminator persistence cannot supply:
    ...
    **Correction, from the player, to what this docstring previously claimed.** It
