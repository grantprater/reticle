...
Every icon the player already marked `enemy` or `question` is presented in turn,
...
reading each cluster by eye -- by me, not by the player. That measures a matcher
...
def main() -> int:
    ...
    # -- they looked done -- so the only icons that reached the player were the ones
