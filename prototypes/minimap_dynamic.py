...
ceiling that no tuning reaches: the player, looking at footage, reports that **some
...
------------------------------------------------------------------
Scored on the 164 labelled frames, and only on blobs this calls red so it
...
(To be exact about what that means, since I described it loosely once and the player
...
Tested because it was the observation and it was worth one measurement.
...
Colour is read from the blob rather than assumed, into the palette the player gave:
red enemy, blue ally, yellow self, green ally ability, and none for the pure
black-and-white glyphs. the player, refining it further: **the team-coloured
...
# the palette. Hue bands in OpenCV's 0-179 space.
...
def searchable(labels, guard_px=LINE_GUARD, guard_boxedges=False, static=None,
    ...
    **`guard_px` is not applied when `static` is given, and that is the
    ...
    line for its neighbour's noise. the player painted over 71% of the box edges and
...
def detect(crop, static_gray, ok_area, diff_min=DIFF_MIN, tophat=True, static_gray2=None):
    ...
    not work at all. Measured at the enemy marks: the raw difference peaks
    ...
    can legitimately have -- unlit and viewcone-lit, the model, confirmed
    ...
    That distinction is load-bearing, found from the player catching a residual
    ...
    for (gpx, gbox) in guards:
      ...
      for th in thresholds:
        ...
        for t, r in rows.items():
            ...
            nred += len(red)
            marks = [m for m in r["marks"]]      # every red thing the player marked
            used = set()
            for m in marks:
                hit = None
