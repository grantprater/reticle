...
The flip is fitted as a control rather than assumed. the player says the scoreboard
...
17.3%, so the orientation claim itself holds -- as-is beats mirrored, as the player
...
the underlying observation is not what failed here. The icon IS a circle at
...
**Still untested: the KILLFEED portrait**, which is the surface the player named
