...
**the domain notes on the minimap, 2026-08-26. None of this is recoverable
...
* **The interior portrait is the same art as the killfeed portrait and the top
  ...
  glyphs: the templates would be taken from the footage on two surfaces
...
* **The local player's icon draws ON TOP of an enemy's** when they overlap.
  Deprioritised on the call: it only happens at extreme short range, where
  the screen detector has a large unambiguous blob and does not need the minimap.

**SETTLED 2026-08-26, against 96 of the hand labels on `a06f04a0059f`.
Two results, and the first one bounds the whole idea.**

**1. The premise HOLDS. the player labelled 96 frames with the screen visible beside
...
enemy ever came into view. the player: *the enemy is often only in view 400 ms or
...
From zero to 79%. The fix came straight from the observation that the ring
...
**Z-order: the local player's icon draws ON TOP of an enemy's.** the player found it;
...
the scoreboard K/D is **19/19**, now in `checks.KNOWN_KD`. Fully
...
killfeed kill (the call, and correct: at a kill an enemy provably existed)
...
**The ring+triangle finder now works** (`minimap_icon_scan.py`). the shape —
...
**The minimap line is parked, and the player is unblocking it on the capture side.**
...
the player recorded one lossless round (`2026-08-26 09-16-10.avi`, Ut Video RGB,
